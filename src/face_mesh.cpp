...
namespace gpolylla {
    ...
    FaceFace::FaceFace(const Face &f)
            : Face(f.vertices[0], f.vertices[1], f.vertices[2]),
              twin(-1) {}
    ...
    FaceTetraMesh::FaceTetraMesh(const TetraMesh &m) : vertices(m.vertices), tetras(m.tetras) {
        ...
        for (int fi = 0; fi < m.faces.size(); fi++) {
            ...
            if (twinFaces.size() > 2) {
                std::cerr << "FACE: Face " << fi << " has " << twinFaces.size() << " twins\n";
                exit(0);
            }
            FaceFace newFace(f);
            if (twinFaces.size() == 2)
